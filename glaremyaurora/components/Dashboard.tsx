...
import SpaceWeatherMap from './SpaceWeatherMap';
...
const Dashboard: React.FC<DashboardProps> = ({ weather, loading, rawText, sources, onRefresh, unitSystem }) => {
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      {/* NEW: 3D Visualization Map */}
      <SpaceWeatherMap weather={weather} />
      ...
      {isSignificantFlare && (
        <div className={`rounded-[2rem] p-5 border-2 relative overflow-hidden ${isXClass ? 'bg-red-900/30 border-red-500' : 'bg-orange-900/30 border-orange-500'} animate-pulse`}>
          <div className="flex items-start gap-4 relative z-10">
            <div className={`w-12 h-12 rounded-full flex-shrink-0 flex items-center justify-center text-2xl border-2 ${isXClass ? 'bg-red-500 text-white border-white' : 'bg-orange-500 text-white border-white'}`}>
              💥
            </div>
            <div className="flex-1">
              <div className="flex justify-between items-start flex-wrap gap-2">
                <h3 className={`text-lg font-black uppercase tracking-wide ${isXClass ? 'text-red-400' : 'text-orange-400'}`}>
                  Solar Flare Detected
                </h3>
                {weather.solarFlare?.time && weather.solarFlare.time !== 'N/A' && (
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-300 bg-slate-900/40 px-2 py-1 rounded-lg border border-slate-700/50">
                    Peak: {weather.solarFlare.time}
                  </span>
                )}
              </div>
              
              <div className="flex items-baseline flex-wrap gap-x-3 mt-1 mb-2">
                 <p className="text-white font-bold text-3xl font-heading leading-none">
                   {weather.solarFlare?.class}
                 </p>
                 {weather.solarFlare?.region && weather.solarFlare.region !== 'Unknown' && (
                   <span className="text-slate-400 text-sm font-medium">
                     from <span className="text-slate-200">Region {weather.solarFlare.region}</span>
                   </span>
                 )}
              </div>

              <p className="text-sm text-slate-200 leading-snug mb-3">
                {weather.solarFlare?.impact}
              </p>

              {weather.solarFlare?.eta && weather.solarFlare.eta !== 'No CME expected' && (
                 <div className="inline-flex items-center gap-2 text-xs font-bold text-teal-200 bg-teal-900/40 px-3 py-1.5 rounded-xl border border-teal-500/30">
                   <span>🚀 CME ETA:</span>
                   <span className="text-white">{weather.solarFlare.eta}</span>
                 </div>
              )}
            </div>
          </div>
